...
class params():
    ...
    #npoints = 100
    #grid is computed
    C_curve = np.loadtxt('C_verlauf.txt')#np.linspace(0, 1, npoints)
    # spatial discretization
    C_profile = np.concatenate([np.zeros(100), C_curve])

    npoints = len(C_profile)
    dx = 1 / npoints
...
class Reaction_Diffusion(object):
    def __init__(self,params, BC='Neumann'):
        ...
        self.BC = BC
        self.C = params.C_profile
        self.C_org = self.C.copy()
        self.beta = 6
        self.alpha = 9 / 11
        self.R_constant = 3
        self.rho = 1/(self.C*self.R_constant)
        self.bfact = 7361
    ...
    def reactionDiffusion(self):
        # 1D diffusion equation
        try:
            self.C = np.matmul(self.A,self.C)

            # compute reaction rates
            self.getReaction()

            # update C field with reaction rates
            self.C = self.C + self.RR+self.dt
    ...
    def advanceReactionDiffusion(self, tsteps = params.tsteps):
    ...
    def getReaction(self):
        exponent = - self.beta * (1 - self.C) / (1 - self.alpha * (1 - self.C))
        self.RR =   (1 - self.C) * np.exp(exponent)
...
Reaction = Reaction_Diffusion(params,BC='Dirichlet')
plt.plot(Reaction.C)

Reaction.advanceReactionDiffusion(tsteps=1000)
plt.plot(Reaction.C)
plt.show()
